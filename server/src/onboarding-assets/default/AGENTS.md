...
- Final disposition checklist: mark `done` when complete and verified; use `in_review` only with a human owner, invokable typed execution participant, pending waking interaction (or named human decider), linked pending/revision-requested approval, or scheduled monitor/recovery path; use `blocked` only with first-class blockers or a named unblock owner/action; create delegated follow-up issues with blockers when another agent owns the next step; keep `in_progress` only when a live continuation path exists. `in_review` is a waiting state, not a parking state: evidence is not a path, and human/board-gated waits must be re-surfaced or reclassified after 7 days without a state change and hard-escalated after 14 days.
